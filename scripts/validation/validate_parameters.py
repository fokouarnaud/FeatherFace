        ...
        if verbose:
            ...
            if not validation_passed:
                ...
                if total_params < target_params:
                    suggested_channel = cfg_mnet['out_channel'] + 2
                    print(f"- Increase out_channel to ~{suggested_channel} (currently {cfg_mnet['out_channel']})")
                    print(f"- Each +1 channel adds ~3K-4K parameters")
                else:
                    suggested_channel = cfg_mnet['out_channel'] - 2
                    print(f"- Decrease out_channel to ~{suggested_channel} (currently {cfg_mnet['out_channel']})")
                    print(f"- Each -1 channel saves ~3K-4K parameters")
                print(f"- SSH modules are largest component (~{(total_params * 0.4)/1000:.0f}K params)")
                print(f"- Parameter per channel scaling: ~{total_params/cfg_mnet['out_channel']/1000:.1f}K per channel")
