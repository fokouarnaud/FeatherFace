#!/usr/bin/env python3
"""
FeatherFace ECA-Net Innovation Training Script

This script trains the ECA-Net innovation model for scientific comparison with CBAM baseline.
Uses cfg_v2_eca_innovation configuration to achieve 475,757 parameters.
...
- Target: Maintain 78.3% Hard WIDERFace AP with 12,907 fewer parameters
"""
...
from data import WiderFaceDetection, detection_collate, preproc, cfg_v2_eca_innovation
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='FeatherFace ECA-Net Innovation Training')
    ...
    # Model arguments
    parser.add_argument('--network', default='eca', 
                       help='Network version: eca for ECA-Net innovation')
...
def main():
    ...
    # Initialize model
    net = FeatherFaceV2ECAInnovation(cfg=cfg)
    print("Printing net...")
    print(net)
    ...
    print(f"Target: 475,757 parameters (ECA-Net innovation)")
    ...
    print('Training FeatherFace ECA-Net innovation on:', dataset.name)
    ...
    for iteration in range(cfg['max_epoch']):
        ...
        if epoch % 10 == 0:
            save_name = os.path.join(args.save_folder, f'featherface_eca_epoch_{epoch}.pth')
    ...
    # Save final model
    final_save_name = os.path.join(args.save_folder, 'featherface_eca_final.pth')
    state_dict = net.state_dict() if not isinstance(net, torch.nn.DataParallel) else net.module.state_dict()
    clean_state_dict = {}
    for key, value in state_dict.items():
        if not (key.endswith('total_ops') or key.endswith('total_params')):
